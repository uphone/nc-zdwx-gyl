...
public class ExcelImportAction extends NCAction {
	...
	public void doAction(ActionEvent arg0) throws Exception {
		// PluginBeanConfigFilePath_Industry0_excelImport
		// nc/ui/ct/purdaily/model/purdaily_excelImport_config.xml
		CardPanelValueUtils utils = new CardPanelValueUtils(editor.getBillCardPanel());
		...
		list = new ZdwxTransExcel().exportListFromExcel(new FileInputStream(
				file), FilenameUtils.getExtension(file.getName()), 0);
		if (list.size() <= 1)
			return;
		list.remove(0);
		...
		for (int i = 0; i < list.size(); i++) {
			...
			String sql = "select pk_material,pk_mattaxes,pk_measdoc,materialtype from bd_material where code='"
					+ row[0] + "' and pk_org='" + pk_org + "'";
			Map<String, String> map = (Map<String, String>) bs.executeQuery(
					sql, new MapProcessor());
			// 税率
			String slSql = "select a.pk_taxcode,b.mattaxescode from bd_taxcode a,bd_mattaxes b where a.mattaxes =b.pk_mattaxes and  b.pk_mattaxes ='"
					+ map.get("pk_mattaxes") + "'";
			Map<String, String> mattaxescodeMap = (Map<String, String>) bs
					.executeQuery(slSql, new MapProcessor());
			...
			if (mattaxescodeMap.get("mattaxescode").equals("CN001")) {// 17%
				pub.setNqtorigtaxprice(new UFDouble(
						Double.valueOf(row[5]) * 0.17));// 含税单价
				pub.setNtaxrate(new UFDouble(17));
				pub.setNtax(new UFDouble(
						pub.getNqtorigprice().toDouble() * 0.17));// 税额
			} else if (mattaxescodeMap.get("mattaxescode").equals("CN002")) {// 13%
				pub.setNqtorigtaxprice(new UFDouble(
						Double.valueOf(row[5]) * 0.13));
				pub.setNtaxrate(new UFDouble(13));
				pub.setNtax(new UFDouble(
						pub.getNqtorigprice().toDouble() * 0.13));// 税额
			} else if (mattaxescodeMap.get("mattaxescode").equals("CN003")) {// 6%
				pub.setNqtorigtaxprice(new UFDouble(
						Double.valueOf(row[5]) * 0.06));
				pub.setNtaxrate(new UFDouble(6));
				pub.setNtax(new UFDouble(
						pub.getNqtorigprice().toDouble() * 0.06));// 税额
			} else if (mattaxescodeMap.get("mattaxescode").equals("CN001")) {// 4%
				pub.setNqtorigtaxprice(new UFDouble(
						Double.valueOf(row[5]) * 0.04));
				pub.setNtaxrate(new UFDouble(4));
				pub.setNtax(new UFDouble(
						pub.getNqtorigprice().toDouble() * 0.04));// 税额
			} else {
			...
			// 价税合计=含税单价*数量
			pub.setNorigtaxmny(new UFDouble(pub.getNqtorigtaxprice().toDouble()
					* pub.getNastnum().toDouble()));
			...
			pub.setVbdef8(String.valueOf(Double.valueOf(row[11])
					* pub.getNastnum().toDouble()));// 成本合价
			pub.setVbdef3(row[12]);// 综合单价
			pub.setVbdef4(String.valueOf(Double.valueOf(row[12])
					* pub.getNastnum().toDouble()));// 综合合价
			...
			String pk_vid = editor.getBillCardPanel().getHeadItem("pk_org_v")
					.getValue();
			String stockSql = "select pk_stockorg  from org_stockorg where pk_vid='"
					+ pk_vid + "'";
			Map<String, String> stockMap = (Map<String, String>) bs
					.executeQuery(stockSql, new MapProcessor());
			pub.setPk_arrvstock(stockMap.get("pk_stockorg"));// 收货库存组织最新版本
			pub.setPk_arrvstock_v(pk_vid);// 收货库存组织
			String finSql = "select pk_financeorg from org_financeorg where pk_vid='"
					+ pk_vid + "'";
			Map<String, String> finMap = (Map<String, String>) bs.executeQuery(
					finSql, new MapProcessor());
...
//			Integer rowCount = editor.getBillCardPanel().getBillModel()
//					.getRowCount();
			bodys[i] = pub;
			utils.addBodyLine("crowno");
			utils.setBodyValueVO(i, pub);
		}
		// editor.getBillCardPanel().getBillModel().addRowAttributeObject(0,
		// "vbdef19", "11");
...
//		this.model.getAppEventListener(nc.ui.pubapp.uif2app.event.card.CardBodyAfterEditEvent.class)[0].
//		System.out.println("12");
	...
	public static String readPath() throws Exception {
		...
		if (fc.getSelectedFile() != null) {
			if (ff.getPath().endsWith(".xlsx") || ff.getPath().endsWith(".xls")
					|| ff.getPath().endsWith(".XLS")
					|| ff.getPath().endsWith(".XLSX")) {
				pathName = ff.getPath();
			} else {
